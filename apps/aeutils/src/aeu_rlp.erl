...
encode(X) when is_integer(X), X >= 0 ->
    %% An integer is a shorthand for a byte array.
    %% NOTE: It will not be decoded into an integer, but into a binary.
    %%       Interpreting the resulting byte array is up to the user.
    encode(binary:encode_unsigned(X));
encode(<<B>> = X) when B =< ?UNTAGGED_LIMIT ->
    %% An untagged value
    X;
